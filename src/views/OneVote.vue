<template>
  <div class="bg-[#f2f4f7]">
    ...
    <div class="relative flex flex-col px-4 h-[100px] py-4 gap-1 ">
      ...
      <span>{{ currentVoteInfo.vote.desc }}<span class="text-[#3a6bea]">{{ currentVoteInfo.vote.multiple !== 0 ? '[多选]' : '[单选]' }}</span></span>
      <span class="absolute block bg-[#3a6bea] text-white w-[45px] h-[45px] rounded-full text-center leading-[45px] text-[40px] right-[16px] top-[10px]"><el-icon><Share /></el-icon></span>
    </div>
    ...
    <div v-if="isButtonShow" class="text-center bg-[#f2f4f7] pb-[20px]" >
      <button
        class="hover:text-[#00bd7e] px-10 py-2 bg-[#3a6bea] text-white rounded mt-[30px] w-7/8 mx-auto ml-auto disabled:bg-[#bcc1cb]" :disabled="isDisabled" @click="submit">
        投票
      </button>
    </div>
  </div>
...
const eachOptionVotes = computed(() => {
  ...
  for (const opt of currentVoteInfo.userVotes) {
    currentOptionVotes[opt.optionId] = currentVoteInfo.userVotes.filter((it: { optionId: unknown }) => it.optionId === opt.optionId)
  }
